...
def ___menu___():
    ...
    if ___menu___ in ['1','01']:
        ...
        print("\n%s[%s1%s]%s Lihat RASHED Ok.txt"%(B,P,B,P))
        print("%s[%s2%s]%s Lihat RASHED Cp.txt"%(B,P,B,P))
...
def ___metode___():
    ...
    if ___metode___ in ['1','01']:
        ...
        print("\n%s[%sâ€¢%s]%s RASHED Ok Tersimpan Di :%s Results/Ok.txt"%(B,P,B,P,H))
        print("%s[%sâ€¢%s]%s RASHED Cp Tersimpan Di :%s Results/Cp.txt"%(B,P,B,P,K))
        ...
        try:
            ...
            exit("%s[%s!%s]%s File Tidak Ada"%(P,M,P,M))
        print("\n%s[%sâ€¢%s]%s RASHED Ok Tersimpan Di :%s Results/Ok.txt"%(B,P,B,P,H))
        print("%s[%sâ€¢%s]%s RASHED OK Tersimpan Di :%s Results/Cp.txt"%(B,P,B,P,K))
        print("%s[%s!%s]%s Gunakan Mode Pesawat Di Angka 1000,2000...\n"%(B,M,B,P))
        with ThreadPoolExecutor(max_workers=35) as (hayuk):
            for user in ids:
                uid, nama = user.split('<=>')
